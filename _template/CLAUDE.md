...
- `/foundations` — **the fluid system's root.** Owns the six shared values (viewport min/max, font-size min/max, ratio min/max) that type + space both derive from; illustrates the cascade. Set the scale here; the other four pages read it. Save + Copy CSS
- `/typography` — fluid **type** scale (Utopia clamp): type-only refinements (steps, weights, line-height, tracking); primitive-step + semantic-role tables. Foundation values are read-only here (tune on /foundations)
