...
export function ShortcutListener({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Global undo/redo for every dialkit adjustment. Skip while a field is
      // focused so text editing keeps its native undo.
      if ((e.metaKey || e.ctrlKey) && !e.altKey) {
        const k = e.key.toLowerCase();
        if (k === 'z') {
          if (isInputFocused()) return;
          e.preventDefault();
          if (e.shiftKey) DialStore.redo();
          else DialStore.undo();
          return;
        }
        if (k === 'y') {
    ...
    // Clicking outside a focused dialkit field blurs it — so its select-all-on-
    // focus selection clears (clicking a non-focusable element otherwise keeps the
    // field focused with its text still highlighted).
    const handleDeselectPointerDown = (e: PointerEvent) => {
      const active = document.activeElement as HTMLElement | null;
      if (!active || (active.tagName !== 'INPUT' && active.tagName !== 'TEXTAREA')) return;
      if (!active.closest('.dialkit-root')) return;
    ...
    window.addEventListener('pointerdown', handleDeselectPointerDown);
    ...
    return () => {
      ...
      window.removeEventListener('pointerdown', handleDeselectPointerDown);
