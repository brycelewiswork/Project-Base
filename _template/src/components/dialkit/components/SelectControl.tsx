...
export function SelectControl({ label, value, options, onChange, help }: SelectControlProps) {
  ...
  return (
    <div className="dialkit-select-row">
      ...
      >
        <span className="dialkit-select-label" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          {label}
          {help && <span onClick={(e) => e.stopPropagation()} style={{ display: 'inline-flex' }}><HelpDot text={help} /></span>}
        </span>
