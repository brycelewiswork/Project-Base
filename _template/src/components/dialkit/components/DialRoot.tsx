...
export function DialRoot({ productionEnabled = isDevDefault, ...rest }: DialRootProps) {
  // Dev-gate lives in this hookless wrapper so DialRootInner's hooks always run in
  // the same order (react-hooks/rules-of-hooks). Upstream dialkit early-returned
  // before its hooks; the split keeps behavior identical and the rule at error.
  if (!productionEnabled) return null;
  return <DialRootInner {...rest} />;
}

function DialRootInner({ position = 'top-right', defaultOpen = true, mode = 'popover', theme = 'system', onOpenChange }: DialRootProps) {
  ...
  // Responsive: below this width the panel becomes a full-width bottom sheet with
  // a backdrop (collapsed = a FAB bubble); above it, the usual floating panel.
  ...
  // Track viewport size for the mobile bottom-sheet mode.
  useEffect(() => {
  ...
  // Collapse the panel by toggling its header (used by the mobile backdrop tap).
  const collapseRoot = useCallback(() => {
    panelRef.current?.querySelector<HTMLElement>('.dialkit-panel-header')?.click();
  }, []);
  ...
  <ShortcutListener>
    <div className="dialkit-root" data-mode={mode} data-theme={theme}>
      {isMobile && !inline && !rootCollapsed && (
        <div className="dialkit-backdrop" aria-hidden="true" onClick={collapseRoot} />
      )}
      <div
        ref={panelRef}
        ...
        onPointerDown={!inline && !isMobile ? handlePointerDown : undefined}
        onPointerMove={!inline && !isMobile ? handlePointerMove : undefined}
        onPointerUp={!inline && !isMobile ? handlePointerUp : undefined}
        onPointerCancel={!inline && !isMobile ? handlePointerUp : undefined}
      >
        {hasMultiplePanels ? (
