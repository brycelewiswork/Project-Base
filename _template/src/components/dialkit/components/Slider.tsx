...
export function Slider({
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (isClickRef.current) {
        // Double-click (two quick clicks) resets to default instead of snapping.
        const now = performance.now();
        if (onReset && now - lastClickUpRef.current < 300) {
          lastClickUpRef.current = 0;
          onReset();
    ...
    <div ref={wrapperRef} className="dialkit-slider-wrapper">
      ...
      >
        ...
        <span ref={labelRef} className="dialkit-slider-label">
          {label}
          {help && <span style={{ marginLeft: 5, display: 'inline-flex', verticalAlign: 'middle' }} onPointerDown={(e) => e.stopPropagation()}><HelpDot text={help} /></span>}
          {shortcut && (
